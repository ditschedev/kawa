import type { BaseLayoutProps } from "fumadocs-ui/layouts/shared";
import { appName, gitConfig } from "./shared";

function FoldLogo(props: React.ComponentProps<"svg">) {
  return (
    <svg viewBox="0 0 40 48" fill="none" aria-hidden="true" {...props}>
      <g fill="currentColor">
        <path
          d="m40 32v-16c0-6.62742-5.3726-12-12-12h-16l-12 12h22c3.3137 0 6 2.6863 6 6v22z"
          opacity=".3"
        />
        <path d="m.0000014 16-.0000014 16c-.00000058 6.6274 5.37258 12 12 12h16l12-12h-20c-4.4183 0-8-3.5817-8-8v-20z" />
      </g>
    </svg>
  );
}

function NavTitle() {
  return (
    <div className="text-fd-foreground flex items-center gap-2">
      <FoldLogo className="h-5 w-4 text-cyan-400" />
      <span className="text-base font-medium tracking-[-0.04em]">{appName}</span>
    </div>
  );
}

export function baseOptions(): BaseLayoutProps {
  return {
    nav: {
      // JSX supported
      title: <NavTitle />,
    },
    githubUrl: `https://github.com/${gitConfig.user}/${gitConfig.repo}`,
  };
}
